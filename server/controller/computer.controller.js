...
async function getAllComputer(req, res) {
  try {
    ...
    return res.status(500).json({
      message:
        "zUne erreur est survenue lors de la récupération des ordinateurs",
    });
  }
}
...
async function createComputer(req, res) {
  try {
    ...
    const addComputer = await Computer.create({
      name: req.query.name,
    });
...
async function editComputer(req, res) {
  try {
    const editComputer = await Computer.findByIdAndUpdate(
...
async function deleteComputer(req, res) {
  try {
    const deleteComputer = await Computer.findByIdAndDelete(
      req.params.computerId
    );
