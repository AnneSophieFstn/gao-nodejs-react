...
import Attribution from "../model/assignment.model.js";
import Computer from "../model/computer.model.js";
import Client from "../model/user.model.js";

async function getAllAssignment(req, res) {
  try {
    const assignments = await Attribution.find().lean();
...
async function getOneAssignment(req, res) {
  const assignment = await Attribution.findById(req.params.assignmentId);
  if (!assignment) {
...
async function createAssignment(req, res) {
  try {
    ...
    const computer = await Computer.findById(req.query.computer_id);
    const client = await Client.findById(req.query.client_id);
    ...
    const createAssignment = Attribution.create({
      horaire: date.toDate(),
      date: req.query.date,
...
async function editAssignment(req, res) {
  const editAssignment = Attribution.findByIdAndUpdate({
...
async function deleteAssignment(req, res) {
  try {
    const deleteAssignment = await Attribution.findByIdAndDelete(
